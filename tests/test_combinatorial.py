...
def harm_context(draw):
    ...
    category = draw(st.sampled_from(list(HarmCategory)))
...
def user_context(draw):
    ...
    consent_model = draw(st.sampled_from(list(ConsentModel)))
...
def weapon_context(draw):
    ...
    w_type = draw(st.sampled_from([None] + list(WeaponType)))
...
class TestCombinatorialLaw1:
    ...
    def test_l1_harm_probability_gradient(self, ctx):
        ...
        # Logic: If prob > 0.6, must refuse. If 0.4-0.6, warn. If < 0.4, permit (unless imminent)
        pipeline = ConstitutionalPipeline()
        # Mocking the internal detection to force specific values would be ideal, 
        # but here we test the decision logic assuming the detector returns these values.
        # For this property test, we assert the threshold logic holds.
        should_refuse = ctx["probability"] > 0.6 or (ctx["probability"] > 0.4 and ctx["imminent"])
        # Note: In a real integration test, we'd inject these values. 
        # Here we assert the mathematical property of the threshold.
    ...
    def test_l1_child_safety_hard_block(self, prompt):
        ...
        # Property: If category is CHILD_SAFETY, result is REFUSE
    ...
    def test_l1_physical_violence_threshold(self, ctx):
        ...
        if ctx["category"] == HarmCategory.PHYSICAL:
            assert ctx["probability"] > 0.3 # Threshold check property
    ...
    def test_l1_psychological_harm_detection(self, ctx):
        ...
        if ctx["category"] == HarmCategory.PSYCHOLOGICAL:
            assert ctx["probability"] > 0.5 # Threshold check property
